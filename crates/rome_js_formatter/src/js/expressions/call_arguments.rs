...
fn can_group_argument(argument: &JsSyntaxNode) -> SyntaxResult<bool> {
    let result = if let Some(object_expression) = JsObjectExpression::cast(argument.clone()) {
        object_expression.members().len() > 0 || object_expression.syntax().has_comments_direct()
    } else if let Some(arrow_expression) = JsArrayExpression::cast(argument.clone()) {
        arrow_expression.elements().len() > 0 || arrow_expression.syntax().has_comments_direct()
...
fn is_react_hook_with_deps_array(node: &JsCallArgumentList) -> SyntaxResult<bool> {
    ...
    let result = if enough_arguments {
        ...
        let no_comments = !node.syntax().has_comments_direct();
